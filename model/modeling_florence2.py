...
class SwitchGate(nn.Module):
    ...
    EDR (Expert Discriminative Routing) uses the 5-class labels to supervise
    the router before top-1 routing. The sparse top-1 mixing path is kept.
    """
    ...
    def forward(self, x: Tensor, use_aux_loss=False, expert_labels: Optional[torch.LongTensor] = None):
        x = x.mean(dim=1)                              # (B, D)
        logits = self.w_gate(x)                        # (B, E)
        gate_scores = F.softmax(logits, dim=-1)        # (B, E)
        ...
        if self.training and expert_labels is not None:
            expert_labels = expert_labels.to(logits.device).long().view(-1)
            valid = (expert_labels >= 0) & (expert_labels < self.num_experts)
            if valid.any():
                route_loss = F.cross_entropy(logits[valid], expert_labels[valid])

        if use_aux_loss and route_loss is None:
            route_loss = torch.tensor(0.0, device=gate_scores.device)

        return gate_scores, route_loss, top_k_indices


class SwitchMoE(nn.Module):
    """
    Switched Mixture of Experts with two supervised auxiliary objectives.

    EDR = router CE + 5-class feature compact/separate loss.
    CCA = image-text consistency alignment loss.
    """
    ...
    def _compute_edr_cluster_loss(self, features: Tensor, expert_labels: Optional[torch.LongTensor]) -> Tensor:
        zero = features.new_zeros(())
        if (not self.training) or expert_labels is None:
            return zero

        expert_labels = expert_labels.to(features.device).long().view(-1)
        valid = (expert_labels >= 0) & (expert_labels < self.num_experts)
        if valid.sum() < 2:
            return zero

        features = features[valid]
        expert_labels = expert_labels[valid]
        unique_labels = torch.unique(expert_labels)
        ...
        for class_id in unique_labels:
            class_features = features[expert_labels == class_id]
            center = class_features.mean(dim=0)
            centers.append(center)
            if class_features.size(0) > 1:
                intra_terms.append((class_features - center).pow(2).sum(dim=-1).mean())
            else:
                intra_terms.append(zero)

        centers = torch.stack(centers, dim=0)
        intra = torch.stack(intra_terms).mean()
        inter = torch.pdist(centers, p=2).pow(2).mean()
        return torch.log1p(intra / (inter + 1e-6))

    def _compute_cca_loss(
            self,
            x: Tensor,
            expert_labels: Optional[torch.LongTensor],
            image_token_length: Optional[int],
    ) -> Tensor:
        zero = x.new_zeros(())
        if (not self.training) or expert_labels is None or image_token_length is None:
            return zero
        if x.size(1) <= image_token_length or image_token_length <= 0:
            return zero

        expert_labels = expert_labels.to(x.device).long().view(-1)
        image_token_length = min(int(image_token_length), x.size(1) - 1)

        img_feat = x[:, 0, :]
        txt_feat = x[:, image_token_length:, :].mean(dim=1)
        similarity = F.cosine_similarity(img_feat, txt_feat, dim=-1)

        # Five-class CCA target design:
        # real: consistent; rumor/text_manipulation: image-text mismatch likely;
        # vision/mixed manipulation are not forced by consistency alignment.
        targets = torch.empty_like(similarity)
        mask = torch.zeros_like(similarity, dtype=torch.bool)
        positive = expert_labels == 0
        negative = (expert_labels == 1) | (expert_labels == 3)
        targets[positive] = 1.0
        targets[negative] = -1.0
        mask = positive | negative
        if not mask.any():
            return zero
        return F.mse_loss(similarity[mask], targets[mask])
    ...
    def forward(
        ...
        gate_scores, route_loss, _ = self.gate(
            x, use_aux_loss=self.use_aux_loss, expert_labels=expert_labels
        )
        expert_outputs = [expert(x) for expert in self.experts]      # list of (B,L,D)
        stacked = torch.stack(expert_outputs, dim=-1)                # (B,L,D,E)

        B, L, D = x.shape
        gate_expand = gate_scores.unsqueeze(1).expand(-1, L, -1)     # (B,L,E)
        moe_output = (stacked * gate_expand.unsqueeze(-2)).sum(dim=-1)  # (B,L,D)

        sample_features = F.normalize(moe_output.mean(dim=1), dim=-1)
        edr_cluster_loss = self._compute_edr_cluster_loss(sample_features, expert_labels)
        if route_loss is None:
            route_loss = x.new_zeros(())
        edr_loss = route_loss + self.edr_cluster_weight * edr_cluster_loss
        cca_loss = self._compute_cca_loss(x, expert_labels, image_token_length)

        return moe_output, {"edr_loss": edr_loss, "cca_loss": cca_loss}


class SwitchTransformerBlock(nn.Module):
    """
    SwitchTransformerBlock is a PyTorch module that represents a single block of the Switch Transformer model.
    """
    ...
    def forward(
        ...
        x, aux_losses = self.ffn(x, expert_labels=expert_labels, image_token_length=image_token_length)
        x = x + add_normed
        x = self.add_norm(x)
        return x, aux_losses
...
class SwitchTransformer(nn.Module):
    ...
    def forward(
        ...
        edr_loss = x.new_zeros(())
        cca_loss = x.new_zeros(())
        loss_count = 0
        for layer in self.layers:
            x, aux_losses = layer(x, expert_labels=expert_labels, image_token_length=image_token_length)
            if aux_losses is not None:
                edr_loss = edr_loss + aux_losses.get("edr_loss", x.new_zeros(()))
                cca_loss = cca_loss + aux_losses.get("cca_loss", x.new_zeros(()))
                loss_count += 1

        if loss_count > 0:
            edr_loss = edr_loss / loss_count
            cca_loss = cca_loss / loss_count

        return x, {"edr_loss": edr_loss, "cca_loss": cca_loss}
...
class Florence2LanguageModel(Florence2LanguagePreTrainedModel):
    ...
    def __init__(self, config: Florence2LanguageConfig):
        ...
        self.moe.apply(self._init_weights)
        self._last_moe_aux_losses = {}
    ...
    def get_last_moe_aux_losses(self):
        return self._last_moe_aux_losses
    ...
    def forward(
        ...
        seq_hidden = encoder_outputs.last_hidden_state
        moe_out, moe_aux_losses = self.moe(
            seq_hidden, expert_labels=expert_labels, image_token_length=image_token_length
        )
        self._last_moe_aux_losses = moe_aux_losses
        combined_features = seq_hidden + moe_out
        final_encoder_hidden_states = self.post_moe_layernorm(combined_features)
        ...
        return Seq2SeqModelOutput(
            ...
            encoder_last_hidden_state=final_encoder_hidden_states,
...
class Florence2LanguageForConditionalGeneration(Florence2LanguagePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Florence2LanguageConfig):
        ...
        # Initialize weights and apply final processing
        self.post_init()
        self.edr_loss_weight = getattr(config, "edr_loss_weight", 0.10)
        self.cca_loss_weight = getattr(config, "cca_loss_weight", 0.05)
        self.last_edr_loss = None
    ...
    def forward(
        ...
        if labels is not None:
            ...
            moe_aux_losses = self.model.get_last_moe_aux_losses()
            zero = masked_lm_loss.new_zeros(())
            edr_loss = moe_aux_losses.get("edr_loss", zero) if moe_aux_losses is not None else zero
            cca_loss = moe_aux_losses.get("cca_loss", zero) if moe_aux_losses is not None else zero
            aux_loss = self.edr_loss_weight * edr_loss + self.cca_loss_weight * cca_loss
            masked_lm_loss = masked_lm_loss + aux_loss

        self.last_edr_loss = edr_loss
...
class Florence2Seq2SeqLMOutput(ModelOutput):
    ...
    edr_loss: Optional[torch.FloatTensor] = None
    cca_loss: Optional[torch.FloatTensor] = None
...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel):
    ...
    def forward(
        ...
        return Florence2Seq2SeqLMOutput(
            loss=loss,
            edr_loss=getattr(self.language_model, "last_edr_loss", None),
            cca_loss=getattr(self.language_model, "last_cca_loss", None),
            aux_loss=getattr(self.language_model, "last_aux_loss", None),
            logits=logits,
            past_key_values=outputs.past_key_values,
            decoder_hidden_states=outputs.decoder_hidden_states,
            decoder_attentions=outputs.decoder_attentions,
